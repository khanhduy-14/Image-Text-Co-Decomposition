...
def train(cfg):
    ...
    logger.info(f"Creating model:{cfg.model.type}/{cfg.method_name}")
    model = build_model(cfg.model)
    model.cuda()

    model.set_train(decoder_only=(cfg.train.ust_steps > 0))
    ...
    do_training(cfg, model, data_loader_train,
                optimizer, lr_scheduler, scaler, val_loaders)
    ...
    for step, samples in enumerate(cyclize(data_loader), config.train.start_step):
        ...
        if step % 100 == 0:
            ...
            if wandb is not None:
                log_stat = {
                    **{f"loss/{loss_name}": loss_value for loss_name, loss_value in loss_dict.items()},
                    **{f"masks/{key}": value for key, value in non_loss_dict.items()}
                }
                log_stat['loss/total_loss'] = loss_meter.val
                log_stat['misc/learning_rate'] = lr
                log_stat['misc/grad_norm'] = grad_norm
                log_stat['misc/memory'] = memory_used
...
def main():
    ...
    if args.resume and args.eval:
        ...
        default_cfg = load_config("configs/default.yml")
        ...
        cfg.method_name = args.method_name
        cfg.checkpoint.resume = args.resume
        cfg.wandb = False
        cfg.evaluate.eval_only = args.eval
        cfg.output = f"output/eval_{cfg.method_name}"
    else:
        cfg = get_config(args)
